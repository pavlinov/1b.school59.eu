...
class hide_my_site{
    ...
    public function verify_login(){
		...
		if( (empty($_COOKIE[$this->get_cookie_name()])) AND (empty($_COOKIE[$this->get_cookie2_name()])) AND (empty($cookie_just_set)) AND ($this->no_admin_bypass()) or ($_GET['hmspreview'] == 'true') ) {
				...
				<html>
				<head>
				<title>".(get_option($this->plugin_slug . '_pagetitle','Password Protected Site'))."</title>"
				.'
	<link rel="icon" href="/wp-content/uploads/2015/08/apple-touch-icon.png" sizes="32x32" />
	<link rel="shortcut icon" href="/wp-content/uploads/2015/08/apple-touch-icon.png" />
	<link rel="apple-touch-icon" href="/wp-content/uploads/2015/08/apple-touch-icon.png">
'
				.$this->get_robots_html().$this->security->get_alert()."<style>";//Begin HTML and login page CSS which can be customized via plugin setting page. also include security alert if applicable
				?>
                body {margin:0px;}
				...
				$template_slug = get_option($this->plugin_slug . "_current_theme", "hmsdiscreet");
